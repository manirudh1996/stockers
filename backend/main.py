...
app = FastAPI(title="Stockers API")
...
def _fetch_stocks():
    ...
    logger.info(f"Fetching {len(yf_syms)} stock symbols from yfinance...")
    try:
        data = yf.download(
            tickers=yf_syms,
            period="5d",
        ...
        for stock in STOCK_META:
            try:
                ...
                if len(close) >= 2:
                    ...
                    hi52 = float(df["High"].dropna().max())
                    lo52 = float(df["Low"].dropna().min())
                    cache[stock["sym"]] = {
                        "price": round(cur, 2),
                        "chg": chg,
                        "high52": round(hi52, 2),
...
def _fetch_indices():
    ...
    logger.info("Fetching index data from yfinance...")
    try:
        data = yf.download(
            tickers=yf_syms,
            period="5d",
        ...
        for idx in INDEX_META:
            try:
                ...
                if len(close) >= 2:
                    ...
                    results.append({**idx, "val": round(val, 2), "chg": chg})
                else:
                    results.append({**idx, "val": 0, "chg": 0})
            except Exception as e:
...
def refresh_cache():
    ...
    logger.info(f"Cache refreshed: {len(_stock_cache)} stocks, {len(_index_cache)} indices")
...
@app.get("/health")
def health():
    return {"status": "ok", "cached_stocks": len(_stock_cache), "last_refresh": _last_refresh}
...
def get_indices():
    ...
    # Return metadata with zero values if not yet fetched
    return {
